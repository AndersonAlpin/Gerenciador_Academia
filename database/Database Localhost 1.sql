CREATE DATABASE  IF NOT EXISTS `sublimemanager` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `sublimemanager`;
...
/*!50106 CREATE*//*!50106 EVENT `eventGerarMensalidades` ON SCHEDULE EVERY 10 SECOND STARTS '2019-12-12 12:47:17' ON COMPLETION NOT PRESERVE ENABLE COMMENT 'Responsável por gerar mensalidades mensalmente' DO BEGIN
...
/*!50001 VIEW `viewMensalidade` AS select `information_schema`.`COLUMNS`.`COLUMN_DEFAULT` AS `valor` from `INFORMATION_SCHEMA`.`COLUMNS` where ((`information_schema`.`COLUMNS`.`TABLE_SCHEMA` = 'sublimemanager') and (`information_schema`.`COLUMNS`.`TABLE_NAME` = 'mensalidades') and (`information_schema`.`COLUMNS`.`COLUMN_NAME` = 'valor')) */;
